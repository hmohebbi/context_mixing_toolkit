...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        # added by Hosein
        # attn
        all_self_attns = () if output_attentions or output_context_mixings.output_attention else None
        # value zeroing
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if output_context_mixings is not None and output_context_mixings.output_value_zeroing:
                    ...
                    for t in range(seq_len): # can be implemented without for but at the cost of memory when having long sequences, so I keep the loop for now
                        alternative_layer_outputs = decoder_layer(
                                            hidden_states=hidden_states,
                                            attention_mask=causal_mask,
                                            position_ids=position_ids,
                                            output_attentions=output_attentions or output_context_mixings.output_attention,
                    ...
                    vz_matrix = torch.divide(vz_matrix, sums, out=torch.zeros_like(vz_matrix), where=~mask, rounding_mode='trunc')
        ...
        return BaseModelOutputWithPast(
            ...
            context_mixings={'attention': all_self_attns, 'value_zeroing': all_value_zeroings},
        )
