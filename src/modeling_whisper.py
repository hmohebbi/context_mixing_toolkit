...
class WhisperAttention(nn.Module):
    ...
    def forward(
        ...
        # Added by Hosein
        if value_zeroing:
            if type(value_zeroing_index) == tuple: # word-level zeroing in encoder
            ...
            else: # token-level zeroing in decoder
                value_states[:, value_zeroing_index] = torch.zeros(value_states[:, value_zeroing_index].size())
        #
...
class WhisperEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, attn_weights, _ = self.self_attn(
            ...
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
            value_zeroing=value_zeroing_type == "encoder",
...
class WhisperDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
            value_zeroing=value_zeroing_type == "decoder",
            value_zeroing_index=value_zeroing_index,
        )
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                output_attentions=output_attentions,
                value_zeroing=value_zeroing_type == "cross",
                value_zeroing_index=value_zeroing_index,
            )
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
