...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        # Added by Hosein
        if value_zeroing: # zeroing value vectors corresponding to the given frame index across all heads
            value_states[:, value_zeroing_index[0]:value_zeroing_index[1]+1] = torch.zeros(value_states[:, value_zeroing_index[0]:value_zeroing_index[1]+1].size())
        #
...
class Wav2Vec2EncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False, value_zeroing_type=None, value_zeroing_index=None):
        attn_residual = hidden_states
        hidden_states, attn_weights, _ = self.attention(
            hidden_states, attention_mask=attention_mask, output_attentions=output_attentions, value_zeroing=value_zeroing_type=="encoder", value_zeroing_index=value_zeroing_index
        )
        hidden_states = self.dropout(hidden_states)
